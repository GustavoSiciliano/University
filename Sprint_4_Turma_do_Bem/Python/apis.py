...
def chamar_api_ia(endpoint, payload=None, metodo='GET'):
    ...
    try:
        ...
        resp = requests.post(url, json=payload, timeout=TIMEOUT_IA) if metodo == 'POST' \
               else requests.get(url, timeout=TIMEOUT_IA)
        resp.raise_for_status()
        data = resp.json()
        ...
        print(f'\n{LARANJA}Sem conexao com {API_IA}.{RESET}')
        return None
    except Exception as e:
...
def verificar_api_ia():
    ...
    print(f'  falta:      {data["modelos"]["falta"]}')
    print(f'  arrecadacao:{data["modelos"]["arrecadacao"]}')
    return True
